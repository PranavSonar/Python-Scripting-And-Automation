...
stage_file_path = str('C:/WorkArea/Python_Home/TEST/{SUB-STREAM} {Devison}ple Files/STAGE/') 		# Stage path is constant for each market
build_file_path = str('C:/WorkArea/Python_Home/TEST/{SUB-STREAM} {Devison}ple Files/build') 		# Get build path randomly
file_count_limit = int(10) 
print file_count_limit

# Go to files directory and read the file
if len(stage_file_path) == 0:
	sys.exit(1)

os.chdir(stage_file_path)
file_count = 0
# print "Searching File..."
for filename in glob('stage.txt'):
    # Delete empty files
    if os.path.getsize(filename) == 0:
        os.remove(filename)
        continue  # Skip to other file

    new_filename = os.path.join(build_file_path, os.path.basename(filename) + ".tmp")
    print filename
    fp = open(filename, 'r')
    ...
    shutil.move(new_filename, os.path.join(build_file_path, filename))
    os.system("chmod 775 " + os.path.join(build_file_path, filename))
    # os.remove(filename)
